...
class AbstractGroup(object):
    ...
    def _constructupdater(self, func=None, updater=None):
        ...
        if func:
            ...
            updtr = Updater(func, self._simulation)
        return updtr
...
class Group(AbstractGroup):
    ...
    simulation : Simulation
        Simulation object to which the group belongs
    func : callable, optional, default : None
    ...
    def __init__(self, simulation, func=None, updater=None, description=None):
        self._description = description
    ...
    def update(self):
        ...
        if self._SystoleUpdater is not None:
            self._SystoleUpdater.update()
        if self._Updater is not None:
            self._Updater.update()
        if self._DiastoleUpdater is not None:
            self._DiastoleUpdater.update()
    ...
    def addfield(self, name, value, func=None, updater=None, description=None, constant=False):
        ...
        [1 1 1 1 1]
        """
        self._fields[name] = Field(self._simulation, value, func=func, updater=updater, description=description, constant=constant)
    ...
    def addgroup(self, name, func=None, updater=None, description=None):
        ...
        Group (My Group):
        """
        self.__dict__[name] = Group(self._simulation, func=func, updater=updater, description=description)
...
class Field(np.ndarray, AbstractGroup):
    ...
    ----------
    simulation : Simulation
        Simulation object to which the field belongs
    value : number, array, string
        Initial value of the field. Needs to have correct type and shape
    func : callable, optional, default : None
    ...
    def __new__(cls, simulation, value, func=None, updater=None, description=None, constant=False):
    ...
    def __array_finalize__(self, obj):
        ...
        self._simulation = getattr(obj, "_simulation", None)
    ...
    def update(self):
        """Function to update the object."""
        if self._Updater is not None:
            self._setvalue(self._Updater.update())
    ...
    def __init__(self, func, simulation):
        if not hasattr(func, '__call__'):
            raise TypeError("<func> is not callable.")
        self._func = func
        self._simulation = simulation
    ...
    def update(self):
        ...
        return self._func(self._simulation)
